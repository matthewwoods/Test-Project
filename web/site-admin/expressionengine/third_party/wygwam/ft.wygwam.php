...
class Wygwam_ft extends EE_Fieldtype {
	...
	private function _field_settings($settings, $matrix = FALSE)
	{
		...
		$r[] = array(
			...
			$config_setting . NBS.NBS . ' <a href="'.BASE.AMP.'C=addons_modules'.AMP.'M=show_module_cp'.AMP.'module=wygwam">'.lang('wygwam_edit_configs').'</a>'
		);
	...
	private function _config_json()
	{
		...
		if ($config['upload_dir'])
		{
			...
			if ($query->num_rows())
			{
				...
				if (! preg_match('/^(\/|\\\|[a-zA-Z]+:)/', $row['server_path'])) $row['server_path'] = FCPATH . $row['server_path'];

				$sess['p'] = $row['server_path'];
				$sess['u'] = $row['url'];
